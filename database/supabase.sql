...
BEGIN
    ...
    SELECT * INTO item_record FROM public.shop_items WHERE id = p_item_id;
    SELECT * INTO user_item_record FROM public.user_items WHERE user_id = p_user_id AND item_id = p_item_id;
    ...
    ELSE
        UPDATE public.user_items SET count = count + 1, updated_at = NOW()
...
DECLARE
    ...
    accumulated_per_tap INTEGER := 0;
    accumulated_max_energy INTEGER := 0;
    accumulated_energy_regen INTEGER := 0;
    i INTEGER;
BEGIN
    -- Get user data
    SELECT * INTO user_record FROM users WHERE user_id = p_user_id;
    ...
    -- Get upgrade bonuses
    SELECT * INTO upgrade_bonuses FROM get_upgrade_bonuses(p_user_id);
    ...
    -- Calculate accumulated bonuses from level 1 up to the new level
    FOR i IN 1..p_new_level LOOP
        SELECT 
            perTapBonus,
            maxEnergyBonus,
            energyRegenBonus 
        ...
        IF level_info IS NOT NULL THEN
            accumulated_per_tap := accumulated_per_tap + level_info.perTapBonus;
            accumulated_max_energy := accumulated_max_energy + level_info.maxEnergyBonus;
            accumulated_energy_regen := accumulated_energy_regen + level_info.energyRegenBonus;
        END IF;
    END LOOP;

    -- Add upgrade bonuses to accumulated values
    accumulated_per_tap := accumulated_per_tap + upgrade_bonuses.per_tap_bonus;
    accumulated_max_energy := accumulated_max_energy + upgrade_bonuses.max_energy_bonus;
    accumulated_energy_regen := accumulated_energy_regen + upgrade_bonuses.energy_regen_bonus;
    ...
    IF user_record.total_farmed >= level_info.requiredFarmed AND user_record.level = p_new_level - 1 THEN
        -- Update user stats for the new level with accumulated bonuses
        -- Ensure minimum values for energy stats
        accumulated_max_energy := GREATEST(100, accumulated_max_energy); -- Minimum max energy is 100
        accumulated_energy_regen := GREATEST(1, accumulated_energy_regen); -- Minimum regen rate is 1
        
        UPDATE users
        SET 
            level = p_new_level,
            per_tap = GREATEST(1, accumulated_per_tap), -- Minimum per tap is 1
            max_energy = accumulated_max_energy,
            energy = accumulated_max_energy, -- Refill energy to new max
            energy_regen_rate = accumulated_energy_regen,
            updated_at = NOW()
        WHERE user_id = p_user_id
        ...
        RETURN json_build_object(
            ...
            'data', json_build_object(
                'level', p_new_level,
                'per_tap', user_record.per_tap,
                'max_energy', user_record.max_energy,
                'energy', user_record.energy,
                'energy_regen_rate', user_record.energy_regen_rate
            )
        );
    ELSE
...
BEGIN
    ...
    spud_per_tap := p_spud_amount / p_tap_count;
...
COPY public.achievements (id, title, description, threshold, type, icon, reward, is_active, created_at) FROM stdin;
1	🏁 First Steps	Earned your first SPUD!	1	balance	🏁	10	t	2025-08-23 13:08:46.275381+00
2	💯 Century Club	Earned 100 SPUD coins!	100	balance	💯	50	t	2025-08-23 13:08:46.275381+00
3	🔥 Thousand Club	Earned 1,000 SPUD coins!	1000	balance	🔥	100	t	2025-08-23 13:08:46.275381+00
4	💎 Ten Thousand Legend	Earned 10,000 SPUD coins!	10000	balance	💎	500	t	2025-08-23 13:08:46.275381+00
5	👥 Social Butterfly	Invited 5 friends	5	referrals	👥	200	t	2025-08-23 13:08:46.275381+00
6	🏆 Mission Master	Completed 10 missions	10	missions	🏆	300	t	2025-08-23 13:08:46.275381+00
7	⚡ Tap Master	Made 1000 taps	1000	taps	⚡	150	t	2025-08-23 13:08:46.275381+00
8	🌟 Elite Farmer	Reached top 10 leaderboard	10	rank	🌟	1000	t	2025-08-23 13:08:46.275381+00
\.


--
-- Data for Name: balance_change_logs; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.balance_change_logs (id, user_id, amount, new_balance, "timestamp", status) FROM stdin;
\.
...
COPY public.missions (id, title, description, reward, type, requirements, is_active, created_at) FROM stdin;
1	🎉 Welcome to SpudVerse	Complete account registration	100	welcome	{"action": "register"}	t	2025-08-23 12:04:27.057159+00
2	📢 Join Telegram Channel	Join our official channel @spudverse_channel	250	social	{"action": "join_channel", "channel": "@spudverse_channel"}	t	2025-08-23 12:04:27.057159+00
4	👥 Invite 5 Friends	Invite 5 friends to join SpudVerse	500	referral	{"count": 5, "action": "invite_friends"}	t	2025-08-23 12:04:27.057159+00
5	🔥 Daily Login	Login daily for 7 days straight	300	daily	{"days": 7, "action": "daily_login"}	t	2025-08-23 12:04:27.057159+00
6	💎 Reach 1K SPUD	Accumulate 1000 SPUD coins	150	achievement	{"action": "reach_balance", "amount": 1000}	t	2025-08-23 12:04:27.057159+00
3	🐦 Follow Twitter	Follow @RealSpudVerse on X (Twitter)	200	social	{"url": "https://x.com/RealSpudVerse", "action": "follow_twitter", "username": "@RealSpudVerse"}	t	2025-08-23 12:04:27.057159+00
\.
...
COPY public.referrals (id, referrer_id, referred_id, bonus_claimed, created_at) FROM stdin;
\.
...
COPY public.shop_items (id, name, cost, profit, scaling, icon, category, created_at) FROM stdin;
1	Old Shovel	10	5	1.5	🪣	Potato Land	2025-08-24 16:01:53.837463+00
2	Farmer's Hut	20	15	1.5	🏡	Potato Land	2025-08-24 16:01:53.837463+00
3	Potato Cart	50	40	1.5	🛒	Potato Land	2025-08-24 16:01:53.837463+00
4	Village Market	100	100	1.5	🥔	Potato Land	2025-08-24 16:01:53.837463+00
5	Potato Mill	250	250	1.5	⚙️	Potato Land	2025-08-24 16:01:53.837463+00
6	Tractor Ride	500	800	1.5	🚜	Potato Land	2025-08-24 16:01:53.837463+00
7	Potato Festival	1200	2500	1.5	🎉	Potato Land	2025-08-24 16:01:53.837463+00
8	Potato Statue	2500	10000	1.5	🗿	Potato Land	2025-08-24 16:01:53.837463+00
9	Potato Shop	5000	20	1.5	🛍	Potato City	2025-08-24 16:01:53.837463+00
10	Street Food Corner	10000	60	1.5	🌭	Potato City	2025-08-24 16:01:53.837463+00
11	Potato Restaurant	25000	200	1.5	🍴	Potato City	2025-08-24 16:01:53.837463+00
12	Delivery Service	50000	700	1.5	🚚	Potato City	2025-08-24 16:01:53.837463+00
13	Potato Factory	120000	3000	1.5	🏭	Potato City	2025-08-24 16:01:53.837463+00
14	Potato Tower	250000	12000	1.5	🗼	Potato City	2025-08-24 16:01:53.837463+00
15	Potato Subway	500000	40000	1.5	🚇	Potato City	2025-08-24 16:01:53.837463+00
16	Potato Skyscraper	1000000	150000	1.5	🏢	Potato City	2025-08-24 16:01:53.837463+00
17	Potato University	2500000	500	1.5	🎓	Potato Nation	2025-08-24 16:01:53.837463+00
18	Potato Bank	5000000	2000	1.5	🏦	Potato Nation	2025-08-24 16:01:53.837463+00
19	Potato Parliament	10000000	8000	1.5	🏛	Potato Nation	2025-08-24 16:01:53.837463+00
20	Potato TV Station	25000000	30000	1.5	📺	Potato Nation	2025-08-24 16:01:53.837463+00
21	National Railway	50000000	120000	1.5	🚂	Potato Nation	2025-08-24 16:01:53.837463+00
22	Potato Military Base	100000000	500000	1.5	🪖	Potato Nation	2025-08-24 16:01:53.837463+00
23	Potato Airport	250000000	2000000	1.5	✈️	Potato Nation	2025-08-24 16:01:53.837463+00
24	Potato Nuclear Plant	500000000	10000000	1.5	☢️	Potato Nation	2025-08-24 16:01:53.837463+00
25	Potato Internet Café	1000000000	50000	1.5	💻	Potato World	2025-08-24 16:01:53.837463+00
26	Potato Silicon Valley	2500000000	200000	1.5	🖥	Potato World	2025-08-24 16:01:53.837463+00
27	Potato Stock Exchange	5000000000	1000000	1.5	📈	Potato World	2025-08-24 16:01:53.837463+00
28	Potato Space Program	10000000000	5000000	1.5	🚀	Potato World	2025-08-24 16:01:53.837463+00
29	Potato AI Lab	25000000000	25000000	1.5	🤖	Potato World	2025-08-24 16:01:53.837463+00
30	Potato Crypto Farm	50000000000	100000000	1.5	⛏	Potato World	2025-08-24 16:01:53.837463+00
31	Potato World Expo	100000000000	500000000	1.5	🌐	Potato World	2025-08-24 16:01:53.837463+00
32	Potato Time Machine	250000000000	2500000000	1.5	⏳	Potato World	2025-08-24 16:01:53.837463+00
33	Potato Moonbase	500000000000	10000000	1.5	🌕	Potato Galaxy	2025-08-24 16:01:53.837463+00
34	Potato Mars Colony	1000000000000	50000000	1.5	🪐	Potato Galaxy	2025-08-24 16:01:53.837463+00
35	Potato Space Station	2500000000000	250000000	1.5	🛰	Potato Galaxy	2025-08-24 16:01:53.837463+00
36	Potato Warp Drive	5000000000000	1200000000	1.5	⚡	Potato Galaxy	2025-08-24 16:01:53.837463+00
37	Potato Black Hole Lab	10000000000000	6000000000	1.5	🌀	Potato Galaxy	2025-08-24 16:01:53.837463+00
38	Potato Galactic Senate	25000000000000	30000000000	1.5	👑	Potato Galaxy	2025-08-24 16:01:53.837463+00
39	Potato Dyson Sphere	50000000000000	150000000000	1.5	☀️	Potato Galaxy	2025-08-24 16:01:53.837463+00
40	Potato Multiverse Portal	100000000000000	999999999999	1.5	🌌	Potato Galaxy	2025-08-24 16:01:53.837463+00
\.
...
COPY public.upgrades (id, name, description, base_cost, cost_multiplier, base_value, value_multiplier, max_level, created_at) FROM stdin;
1	per_tap	Increase SPUD earned per tap	100	1.8	1	1	20	2025-08-24 11:39:35.59988+00
2	max_energy	Increase maximum energy capacity	150	2	100	1.2	15	2025-08-24 11:39:35.59988+00
3	energy_regen_rate	Increase energy regeneration speed	200	2.2	1	1	10	2025-08-24 11:39:35.59988+00
\.
...
COPY public.user_achievements (id, user_id, achievement_id, unlocked_at, claimed, created_at) FROM stdin;
83	930339179	1	2025-08-25 07:37:43.872683+00	f	2025-08-25 07:37:43.872683+00
\.
...
COPY public.user_items (id, user_id, item_id, count, updated_at) FROM stdin;
\.
...
COPY public.user_missions (id, user_id, mission_id, completed, claimed, completed_at, created_at) FROM stdin;
133	930339179	1	t	f	2025-08-25 07:37:31.697+00	2025-08-25 07:37:31.80856+00
\.


--
-- Data for Name: user_upgrades; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public
4	add-migrations-rls	427c5b63fe1c5937495d9c635c263ee7a5905058	2025-07-27 04:37:12.369343
5	add-size-functions	79e081a1455b63666c1294a440f8ad4b1e6a7f84	2025-07-27 04:37:12.372788
6	change-column-name-in-get-size	f93f62afdf6613ee5e7e815b30d02dc990201044	2025-07-27 04:37:12.388585
7	add-rls-to-buckets	e7e7f86adbc51049f341dfe8d30256c1abca17aa	2025-07-27 04:37:12.39215
8	add-public-to-buckets	fd670db39ed65f9d08b01db09d6202503ca2bab3	2025-07-27 04:37:12.395379
9	fix-search-function	3a0af29f42e35a4d101c259ed955b67e1bee6825	2025-07-27 04:37:12.400989
10	search-files-search-function	68dc14822daad0ffac3746a502234f486182ef6e	2025-07-27 04:37:12.410315
11	add-trigger-to-auto-update-updated_at-column	7425bdb14366d1739fa8a18c83100636d74dcaa2	2025-07-27 04:37:12.416336
12	add-automatic-avif-detection-flag	8e92e1266eb29518b6a4c5313ab8f29dd0d08df9	2025-07-27 04:37:12.423815
13	add-bucket-custom-limits	cce962054138135cd9a8c4bcd531598684b25e7d	2025-07-27 04:37:12.428776
14	use-bytes-for-max-size	941c41b346f9802b411f06f30e972ad4744dad27	2025-07-27 04:37:12.433227
15	add-can-insert-object-function	934146bc38ead475f4ef4b555c524ee5d66799e5	2025-07-27 04:37:12.486713
16	add-version	76debf38d3fd07dcfc747ca49096457d95b1221b	2025-07-27 04:37:12.489763
17	drop-owner-foreign-key	f1cbb288f1b7a4c1eb8c38504b80ae2a0153d101	2025-07-27 04:37:12.492298
18	add_owner_id_column_deprecate_owner	e7a511b379110b08e2f214be852c35414749fe66	2025-07-27 04:37:12.496234
19	alter-default-value-objects-id	02e5e22a78626187e00d173dc45f58fa66a4f043	2025-07-27 04:37:12.504066
20	list-objects-with-delimiter	cd694ae708e51ba82bf012bba00caf4f3b6393b7	2025-07-27 04:37:12.507368
21	s3-multipart-uploads	8c804d4a566c40cd1e4cc5b3725a664a9303657f	2025-07-27 04:37:12.517475
22	s3-multipart-uploads-big-ints	9737dc258d2397953c9953d9b86920b8be0cdb73	2025-07-27 04:37:12.555959
23	optimize-search-function	9d7e604cddc4b56a5422dc68c9313f4a1b6f132c	2025-07-27 04:37:12.564617
24	operation-function	8312e37c2bf9e76bbe841aa5fda889206d2bf8aa	2025-07-27 04:37:12.567758
25	custom-metadata	d974c6057c3db1c1f847afa0e291e6165693b990	2025-07-27 04:37:12.57068
\.
...
--
-- Name: balance_change_logs_id_seq; Type: SEQUENCE SET; Schema: public; Owner: postgres
--

SELECT pg_catalog.setval('public.balance_change_logs_id_seq', 1, false);
...
SELECT pg_catalog.setval('public.referrals_id_seq', 17, true);
...
SELECT pg_catalog.setval('public.shop_items_id_seq', 40, true);
...
SELECT pg_catalog.setval('public.upgrades_id_seq', 3, true);
...
SELECT pg_catalog.setval('public.user_achievements_id_seq', 83, true);
...
SELECT pg_catalog.setval('public.user_items_id_seq', 10, true);
...
SELECT pg_catalog.setval('public.user_missions_id_seq', 133, true);


--
-- Name: user_upgrades_id_seq; Type: SEQUENCE SET; Schema: public; Owner: postgres
--

SELECT pg_catalog.setval('public.user_upgrades_id_seq', 3, true);
